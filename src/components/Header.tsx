...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (
        showUserMenu &&
        target &&
        !(target as HTMLElement).closest(".user-menu-container")
      ) {
  ...
  const navigationItems = [
    { label: "Home", href: "#home", ariaLabel: "Navigate to homepage" },
    {
      label: "About",
      href: "#about",
      ariaLabel: "Learn about Namami Enterprises",
    },
    {
      label: "Products",
      href: "#products",
      ariaLabel: "View agarbatti products",
    },
    {
      label: "Services",
      href: "#services",
      ariaLabel: "Our manufacturing services",
    },
    { label: "Blog", href: "#blogs", ariaLabel: "Read our latest blogs" },
    {
      label: "Why Choose Us",
      href: "#why-choose-us",
      ariaLabel: "Why choose Namami Enterprises",
    },
  ...
  const scrollToHash = useCallback((hash: string, closeMenu = false) => {
    ...
    if (!target) {
      for (const id of Array.from(triedIds)) {
        target = document.querySelector(
          `[data-section="${id}"], [data-anchor="${id}"]`,
        );
        if (target) break;
      }
    }
    ...
    const absoluteTop =
      window.scrollY + targetRect.top - headerHeight - extraOffset;
  ...
  const handleNavClick = (
    ...
    // Agar homepage par nahi ho
    if (window.location.pathname !== "/") {
      navigate(`/${href}`);
      return;
    }

    // Homepage par ho to scroll karo
  ...
  return (
    <>
      ...
      <div
        className="hidden lg:block bg-gradient-to-r from-orange-600 via-orange-500 to-amber-500 text-white"
        role="complementary"
      >
            ...
            <address className="flex items-center gap-6 not-italic">
              {contactInfo.map((item, index) => (
                ...
                >
                  ...
                  <span className="font-medium" itemProp="contactType">
                    {item.text}
                  </span>
                </a>
              ))}
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-20">
            ...
            >
              ...
              <div>
                <h1
                  className="text-2xl font-extrabold bg-gradient-to-r from-orange-600 via-orange-500 to-amber-500 bg-clip-text text-transparent group-hover:from-orange-700 group-hover:to-amber-600 transition-all duration-300"
                  itemProp="name"
                >
                  Namami Enterprises
                </h1>
                <p
                  className="text-xs text-gray-600 font-semibold tracking-wide"
                  itemProp="slogan"
                >
            ...
            >
              {navigationItems.map((item) => (
                ...
                >
                  ...
                  <span
                    className="absolute bottom-1 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-gradient-to-r from-orange-600 to-amber-500 group-hover:w-3/4 transition-all duration-300"
                    aria-hidden="true"
                  />
                </a>
              ))}
            ...
            <div className="hidden lg:flex items-center gap-3">
              {user ? (
                <div className="relative user-menu-container">
                  ...
                  >
                    <div className="relative">
                      {user.photoURL ? (
                        <img
                          src={user.photoURL}
                          alt={`${getUserDisplayName()}'s profile picture`}
                  ...
                  {showUserMenu && (
                    <div
                      className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 animate-in slide-in-from-top-5 duration-200 z-50"
                      role="menu"
                ...
                <>
                  <Button
                    ...
                    aria-label="Call now at +91 7067449775"
                  >
          ...
          {isMenuOpen && (
            <div
              id="mobile-menu"
              className="lg:hidden py-4 border-t border-gray-200 animate-in slide-in-from-top duration-300"
              role="navigation"
